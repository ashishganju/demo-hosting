...
if($result->num_rows > 0){
	while($row = $result->fetch_assoc()){
		...
		$message="Your activation link is:http://localhost/forgott/Reset_pass.php?token=$id_encode";
        mail($email, "Reset password link", $message);
		echo 'password reset link sent succesfully';
        // include 'index.php';
		// $link = "<a href='Reset_pass.php?MnoQtyPXZORTE=$id_encode' class='btn btn-info btn-sm'>$email</a>";
	}
	}else{
